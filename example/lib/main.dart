...
class _MyAppState extends State<MyApp> {
  ...
  void closeOverlay(String entryPointName) async {
    var ids = _overlayWindowIds
        .where((element) => element.startsWith(entryPointName))
        .toList();
  ...
  void updateDragble(String entryPointName) {
    var ids = _overlayWindowIds
        .where((element) => element.startsWith(entryPointName))
        .toList();
    if (dragebleClickTime > 1) {
      dragebleClickTime = 0;
    }
    ids.forEach((element) async {
      if (dragebleClickTime == 0) {
        _overlayWindowsPlugin.updateDragble(element, false);
      } else if (dragebleClickTime == 1) {
        _overlayWindowsPlugin.updateDragble(element, true);
      }
    });
    dragebleClickTime++;
  }
  ...
  void increaseSize() {
    setState(() {
      initWidth += 20;
      initHeight += 20;
      _overlayWindowsPlugin.resize(
          _overlayWindowIds.first, initWidth, initHeight);
    });
  }
              ...
              SingleChildScrollView(
                ...
                child: Row(
                  ...
                  children: [
                    ...
                    ElevatedButton(
                      onPressed: () {
                        updateDragble("overlayMain1");
                      },
                      child: const Text("updateDragble"),
                    ),
                    ElevatedButton(
                      onPressed: () {
                        increaseSize();
              ...
              Expanded(
                child: ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    return Text(
                        '${message[index].overlayWindowId}: ${message[index].message}');
                  },
                ),
              ),
            ],
