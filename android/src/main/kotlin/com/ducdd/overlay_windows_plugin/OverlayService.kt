...
class OverlayService : Service(), java.io.Serializable {
    companion object {
        ...
        const val OVERLAY_WINDOW_DRAGBLE = "overlay_window_dragble"
...
//            return START_STICKY
            ...
            } else if (action == "updateDragble") {

                var existView: OverlayView? =
                    _views.find { x -> x.viewId == viewId } ?: throw Exception("Inactive")

                var dragble = false;
                if (intent.extras!!.containsKey(OVERLAY_WINDOW_DRAGBLE)) {
                    dragble = intent.getBooleanExtra(OVERLAY_WINDOW_DRAGBLE,false)!!
                }

                Log.d("dragble", "set dragble $dragble to view $viewId")

                existView?.updateDragble(dragble);
