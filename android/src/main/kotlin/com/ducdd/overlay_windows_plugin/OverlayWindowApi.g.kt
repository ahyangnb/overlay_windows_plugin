...
interface OverlayWindowApi {
  ...
  fun updateEnableDrag(overlayWindowId: String, enableDrag: Boolean)
  ...
  companion object {
    ...
    fun setUp(binaryMessenger: BinaryMessenger, api: OverlayWindowApi?) {
      ...
      run {
        val channel = BasicMessageChannel<Any?>(binaryMessenger, "dev.flutter.pigeon.OverlayWindowApi.updateEnableDrag", codec)
        if (api != null) {
          channel.setMessageHandler { message, reply ->
            val args = message as List<Any?>
            val overlayWindowIdArg = args[0] as String
